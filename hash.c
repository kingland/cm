...
u32 
cm_crc32(void const *data, isize len) {
	isize remaining;
	u32 result = ~(cast(u32)0);
	u8 const *c = cast(u8 const *)data;
	for (remaining = len; remaining--; c++) {
		result = (result >> 8) ^ (CM__CRC32_TABLE[(result ^ *c) & 0xff]);
	}
	return ~result;
}

u64 
cm_crc64(void const *data, isize len) {
	isize remaining;
	u64 result = ~(cast(u64)0);
	u8 const *c = cast(u8 const *)data;
	for (remaining = len; remaining--; c++) {
		result = (result >> 8) ^ (CM__CRC64_TABLE[(result ^ *c) & 0xff]);
	}
	return ~result;
}

u32 
cm_fnv32(void const *data, isize len) {
	isize i;
	u32 h = 0x811c9dc5;
	u8 const *c = cast(u8 const *)data;
...
u64 
cm_fnv64(void const *data, isize len) {
	isize i;
...
u32 
cm_fnv32a(void const *data, isize len) {
	isize i;
	u32 h = 0x811c9dc5;
	u8 const *c = cast(u8 const *)data;
...
u64 
cm_fnv64a(void const *data, isize len) {
	isize i;
	u64 h = 0xcbf29ce484222325ull;
	u8 const *c = cast(u8 const *)data;
...
cm_inline u32 
cm_murmur32(void const *data, isize len) { 
    return cm_murmur32_seed(data, len, 0x9747b28c); 
}

cm_inline u64 
cm_murmur64(void const *data, isize len) { 
    return cm_murmur64_seed(data, len, 0x9747b28c); 
}

u32 
cm_murmur32_seed(void const *data, isize len, u32 seed) {
...
u64 
cm_murmur64_seed(void const *data_, isize len, u64 seed) {
